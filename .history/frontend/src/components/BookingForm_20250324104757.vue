...
const fetchUnavailableDates = async () => {
  try {
    ...
    // disabledDates.value = response.data.map(entry => new Date(entry.date + "T00:00:00Z"));
...
const calculateBasePrice = () => {
  if (!selectedDates.value[0] || !selectedDates.value[1]) return 0;
  const nights = (selectedDates.value[1] - selectedDates.value[0]) / (1000 * 60 * 60 * 24);
...
watch(selectedDates, (newDates) => {
  ...
  const maxCheckoutTime = new Date(checkIn);
  maxCheckoutTime.setDate(maxCheckoutTime.getDate() + 3);

  if (checkOut > maxCheckoutTime) {
    selectedDates.value[1] = maxCheckoutTime;
    userMessage.value = "⚠️ Maximal 3 Nächte erlaubt. Check-out wurde automatisch angepasst!";
  } else {
    userMessage.value = "";
  }
});
  ...
  <div class="booking-form">
    ...
    <div v-if="step === 1">
      ...
      <Datepicker 
        ...
        :utc= "preserve"
        :range="{ noDisabledRange: true, minRange: 1, maxRange: 2, showLastInRange: true, partialRange: false }"
        ...
        auto-apply
      />
      <p v-if="userMessage" class="warning-message">{{ userMessage }}</p>
