...
<hr>
  ...
  <div v-for="(car, index) in assignedSpots" :key="index" class="car-container">
    ...
    <div class="form-row">
      ...
      <input type="number" v-model="car.guestsBelow14" min="0" :class="{'error-border': errorFields.includes(`Gäste 0-13 für Stellplatz ${index + 1}`)}" placeholder="0" />
    </div>

    <div class="form-row">
      <label>Gäste 14+ Jahre:</label>
      <input type="number" v-model="car.guestsAbove14"  :class="{'error-border': errorFields.includes(`Gäste 14+ für Stellplatz ${index + 1}`)}" placeholder="1" />
    </div>
