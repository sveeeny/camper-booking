<template>
  <!-- Overlay-Hintergrund (optional, nur für mobiles Feeling) -->
  <div class="fixed inset-0 bg-black/30 z-40" @click.self="emitClose"></div>

  <!-- Slide-In Panel -->
  <div
    class="fixed right-0 top-0 h-full w-full sm:w-[540px] bg-white dark:bg-slate-900 shadow-lg z-50 transform transition-transform duration-300 ease-in-out"
    :class="{ 'translate-x-0': true, 'translate-x-full': !show }"
  >
    <!-- Zurück-Button -->
    <div class="flex justify-between items-center p-4 border-b border-slate-200 dark:border-slate-700">
      ...
      <button
        @click="emitClose"
        class="text-slate-500 hover:text-slate-800 dark:hover:text-white transition"
      >
        ✕
      </button>
    </div>
    ...
    {{ updatingStatus ? 'Aktualisiere…' : 'Als bezahlt markieren' }}
        ...
        >
          <div
            ...
            :key="car.carPlate"
            class="bg-white dark:bg-slate-800 p-4 rounded shadow border border-slate-200 dark:border-slate-700"
          >
...
import { fetchBookingById } from '@/composables/Host/useHostBookings';
import { markBookingAsPaid } from '@/composables/Host/useHostBookings';
...
const markAsPaid = async () => {
  ...
  const confirmed = confirm(`Diese Buchung wirklich als bezahlt markieren?`);
  if (!confirmed) return;
  ...
  try {
    ...
    booking.value.status = 'paid'; // 🟢 UI sofort aktualisieren
  } catch (err) {
...
const booking = ref<HostBooking | null>(null);
const show = ref(false);

const emitClose = () => {
  show.value = false;
  setTimeout(() => emit('close'), 300); // ⏳ auf Animation warten
};
